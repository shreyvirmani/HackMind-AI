...
class SubscriptionRepository:
    ...
    def reset_window_if_needed(
        ...
        window_start = sub.generation_window_start or sub.created_at or datetime.utcnow()

        if datetime.utcnow() - window_start >= timedelta(days=GENERATION_WINDOW_DAYS):
    ...
    def increment_generations_used(self, db: Session, user_id: str) -> Subscription:
    ...
    def upgrade_to_pro(
        self, db: Session, user_id: str, months: int = 1
    ) -> Subscription:
        sub = self.get_or_create(db, user_id)
        sub.plan = "pro"
        sub.status = "active"
    ...
    def expire_if_needed(self, db: Session, sub: Subscription) -> Subscription:
        """Pro plans are time-boxed -- if expires_at has passed, drop
        back to Free instead of silently keeping Pro access forever."""
...
class PaymentRepository:
    ...
    def get_by_order_id(self, db: Session, razorpay_order_id: str) -> Payment:
        return (
            db.query(Payment)
    ...
    def mark_paid(
        self, db: Session, razorpay_order_id: str, razorpay_payment_id: str
    ) -> Payment:
        payment = self.get_by_order_id(db, razorpay_order_id)
        if payment:
            payment.status = "paid"
    ...
    def mark_failed(self, db: Session, razorpay_order_id: str) -> Payment:
        payment = self.get_by_order_id(db, razorpay_order_id)
        if payment:
